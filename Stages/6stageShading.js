...
function transformShading(){
  for (let i = 0; i < shadingObject.vertices.length; i++) {
    shadingObject.vertices[i].mul(50);
    shadingObject.vertices[i].add(new Vector(250, 250, 0));
  }
}

function reverseTransformShading(){
  for (let i = 0; i < shadingObject.vertices.length; i++) {
    shadingObject.vertices[i].add(new Vector(-250, -250, 0));
    shadingObject.vertices[i].mul(1/50);
  }
}

function continueRotationShading(){
  for (let i = 0; i < shadingObject.vertices.length; i++) {
    shadingObject.vertices[i].rotateX(-0.01);
    shadingObject.vertices[i].rotateZ(-0.01);
  }
}


canvasShading = document.getElementById('shadingCanvas');
ctxShading = canvasShading.getContext('2d');
...
function renderShading(){
  ...
  for (let i = 0; i < shadingObject.faces.length; i++) {
    let face = shadingObject.faces[i];


    let p1 = shadingObject.vertices[face.points[0]];
    let p2 = shadingObject.vertices[face.points[1]];
    let p3 = shadingObject.vertices[face.points[2]];
        ...
        for (let y = yMin; y <= yMax; y++) {
            ...
            if(0 <= l1 && 0 <= l2 && 0 <= l3){
              ...
              if(depth > zBuffer[zIndex]){
                ...
                ctxShading.fillStyle = finalColor;
                ctxShading.fillRect(x, y, 1, 1);
              }
            }
        }
      }
...
pauseShading = false;
canvasShading.addEventListener('click',() =>{pauseShading = !pauseShading}, false);

function loopShading(){
  if(!pauseShading){

    ctxShading.clearRect(0, 0, 500, 500);
    transformShading();
    renderShading();
    reverseTransformShading();
    continueRotationShading();
  }
}

loopShading();
pauseShading = true;
setInterval(loopShading, 33);
